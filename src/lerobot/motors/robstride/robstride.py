...
# TODO(Virgile) : add the defaiult mode control , only the MIS protocole isimplemented for now
...
class RobstrideMotorsBus(MotorsBusBase):
    ...
    def _recv_status_via_clear_fault(self, expected_recv_id: Optional[int] = None, timeout: float = 0.001):
        ...
        while time.time() - start_time < timeout:
            msg = self.canbus.recv(timeout=0.0001)
            if not msg:
                continue

            if expected_recv_id is not None and msg.arbitration_id != expected_recv_id:
                continue
    ...
    def _switch_operation_mode(self,motor, mode: ControlMode) -> None:
        ...
        motor_id = self._get_motor_id(motor_name)
        recv_id = self._get_motor_recv_id(motor_name)
        data = [0xFF] * 6 + [mode] + [0xFC]
        msg = can.Message(arbitration_id=motor_id, data=data, is_extended_id=False)
        self.canbus.send(msg)   
        msg= self._recv_motor_response(expected_recv_id=recv_id)
        if msg is not None:
            self.operationlMode[motor_name]=mode    
    ...
    def _decode_motor_state(self, data: bytes) -> Tuple[float, float, float, int, int]:
        """
        ...
        motor_id = data[0]
        motor_name = self._get_motor_name_by_id(motor_id)
        ...
        *,
        ...
        for motor in motors:
            try:
                ...
                self.currentPosition[motor]=position_degrees
                self.currentVelocity[motor]=velocity_deg_per_sec
                self.currentTorque[motor]=torque
                self.currentTemperature[motor]=(t_mos + t_rotor)/2
                self.last_feedback_time[motor]=time.time()
    ...
    def sync_write(
        ...
        *,
        ...
        if data_name == "Goal_Position":
            ...
            responses = self._recv_all_responses(expected_recv_ids, timeout=0.002)  # 2ms timeout
            self._decode_motor_state([msg.data for msg in responses.values()])
        else:
